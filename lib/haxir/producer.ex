defmodule Haxir.Producer do
  ...
  defp convert_event(%{"event" => event, "args" => args}) do
    case event do
      "onPlayerJoin" ->
        {:player_joined, args["player"]}

      "onPlayerLeave" ->
        {:player_left, args["player"]}

      "onPlayerChat" ->
        {:new_message, {args["player"], args["message"]}}
      ...
      "onPlayerBallKick" ->
        {:ball_kicked, args["player"]}

      "onTeamGoal" ->
        {:team_scored, args["team"]}

      "onGameStart" ->
        {:game_started, args["byPlayer"]}

      "onGameStop" ->
        {:game_stopped, args["byPlayer"]}

      "onPlayerAdminChange" ->
        {:admin_changed, {args["changedPlayer"], args["byPlayer"]}}

      "onPlayerTeamChange" ->
        {:team_changed, {args["changedPlayer"], args["byPlayer"]}}

      "onPlayerKicked" ->
        {:player_kicked, {args["kickedPlayer"], args["reason"], args["ban"], args["byPlayer"]}}

      "onGamePause" ->
        {:game_paused, args["byPlayer"]}

      "onGameUnpause" ->
        {:game_unpaused, args["byPlayer"]}

      "onPositionsReset" ->
        {:positions_reseted, nil}

      "onPlayerActivity" ->
        {:player_activity, args["player"]}

      "onStadiumChange" ->
        {:stadium_changed, {args["newStadiumName"], args["byPlayer"]}}
