...
function readySocket() {
  if(!window.ready) window.ready = 0
  window.ready++

  if(window.ready >= 2) send("ready")
}
...
function setEvents() {
  ...
  window.room.onGameTick = () => {
    ...
    tickTime = !tickTime // from 60hz to 30hz
...
function handleMessage(message : string, args : any) {
  ...
  else if(message == "send_announcement") {
